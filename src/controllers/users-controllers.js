...
module.exports = {
  ...
  otpVerify: async (req, res) => {
    ...
    const tmpOtp = await userModels.getOtp(number).then(result => {
      return result[0];
    });
    if (tmpOtp) {
      if (tmpOtp.otp == otp) {
        ...
        userModels.getUser(number).then(result => {
          if (result.length) {
  ...
  buyPackage: async (req, res) => {
    ...
    const targetPackage = await packagesModels
      .getPackage(packageID)
      .then(result => {
        return result[0];
      });

    const user = await userModels.getUser(number).then(result => {
      return result[0];
    });
      ...
      if (found) {
        ...
        currentPackage.packageItems.map((item, index) => {
          if (currentPackage.packageItems[index].value > 0) {
            currentPackage.packageItems[index].value +=
              targetPackage.packageItems[index].value;
            currentPackage.packageItems[index].remaining +=
              targetPackage.packageItems[index].remaining;
          }
        });
        ...
        userModels.editPackage(number, currentPackages).then(result => {
          formResponse.success(res, 200, currentPackages);
        });
      } else {
        //add new package
        await userModels.addPackage(number, targetPackage).then(result => {
  ...
  removePackage: async (req, res) => {
    ...
    userModels.removePackage(number, packageID).then(result => {
      const data = {
        number,
        packageID
      };
  ...
  decreaseDay: async (req, res) => {
    const users = await userModels.getAllUsers().then(result => {
      return result;
    });
